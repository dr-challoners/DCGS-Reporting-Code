...
<!-- Name: sliders-bw-plus-results_DEV.xsl -->
					...
					<fo:table-body>
						...
						<fo:table-row>
							<fo:table-cell padding="1pt" text-align="center">
								...
								<xsl:for-each select="Skills/Skill[Name='Effort']">
									<fo:instream-foreign-object>
										<xsl:call-template name="Slider">
											<xsl:with-param name="_displayMode" select="'bw'" />
											<xsl:with-param name="_showAllBands" select="true()" />
						...
						<xsl:for-each select="Skills/Skill[Name!='Attainment' and Name!='Commitment'and Name!='Effort']">
							<fo:table-row>
